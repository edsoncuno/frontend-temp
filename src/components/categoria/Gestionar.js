...
export default class Gestionar extends React.Component {
    ...
    render() {
        ...
        const actions = (data) => {
            return (
                <React.Fragment>
                    <Button title="Eliminar tipo de item" icon="pi pi-times" className='m-2 bg-pink-500' onClick={() => { eliminar(data._id) }} />
                </React.Fragment>
            );
        }
        ...
        const header = () => {
            return (
                <React.Fragment>
                    <Button title="Agregar un nuevo tipo de item" icon="pi pi-plus" className='m-2' onClick={crear} />
        ...
        const actualizar = async () => {
            ...
            this.setState({ list: await getCategorias() });
            this.setState({ loading: false });
        }
